...
interface PostFXPassesProps {
  bloom: boolean;
  grade: boolean;
}

/**
 * The actual effect passes. Split from `PostFX` so the two postprocessing
 * libraries live in their own chunk and are fetched only when a quality tier
 * enables an effect — which, at present, none does.
 *
 * ── Restrained to the point of near-absence, for a reason ─────────────────────
 *
 * The first version used `mipmapBlur` bloom at a 0.72 luminance threshold. With
 * additive particles and no tone mapping, the dense core accumulates luminance far
 * above 1.0, so almost the whole field cleared the threshold — and mipmap
 ...
 *
 * That version still produced a pyramid of nested square halos on real hardware,
 * which is why every tier now has `bloom: false`. This is kept as a working
 * starting point, not as a recommendation. Read the note in lib/field/config.ts
 ...
 */
export default function PostFXPasses({ bloom, grade }: PostFXPassesProps) {
