...
| **JavaScript** disabled or failed | All content renders. Reveals use `immediateRender: false`, so the `opacity: 0` from-state is applied only when an animation actually starts — content that is never animated is never hidden. A started tween also carries a 4s safety that force-completes it, so a stalled `requestAnimationFrame` cannot leave text half-faded. (An earlier version applied the from-state on creation and did fail this way: with rAF starved, the whole hero rendered blank.) Navigation links are real anchors. The mobile menu is the one loss: primary nav is unreachable below 1024px, but the footer carries every route |
